...
{
    private static final int DEFAULT_MAX_LINES = 100000000;			//Max lines per file
    private static final int DEFAULT_MAX_LINE_LENGTH = 128*1024;	//Max bytes per line: 128kb
    ...
    {
        super(reader);
        if ((maxLines<=0) || (maxLineLen<=0)) throw new IllegalArgumentException("O número máximo de linhas e o tamanho mínimo da linha precisam ser maiores que 0");
    ...
    public String readLine() throws IOException
    {
        ...
        if (currentLine > readerMaxLines) throw new IOException("Número máximo de linhas excedido.");
        currentLine++;
        ...
        if (currentPos >= readerMaxLineLen) {
            throw new IOException("O tamanho da linha foi excedido. Atualmente esse valor máximo é de " + readerMaxLineLen + " bytes.");
        }
